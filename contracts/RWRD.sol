...
contract RWRD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RWRDDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        dividendTracker = new RWRDDividendTracker(
            payable(this),
            router,
...
contract RWRDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        RWRDContract = RWRD(mainContract);
    ...
    function _transfer(
        ...
        require(false, "RWRD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "RWRD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RWRD contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(RWRDContract);

            uint256 prevBalance = RWRDContract.balanceOf(address(this));
            ...
            {
                uint256 received = RWRDContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RWRDContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
