...
contract RWRD_B is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RWRD_BDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        dividendTracker = new RWRD_BDividendTracker(
            payable(this),
            router,
            dividendTrackerToken,
            "DIVTRACKER",
            "DIVT"
        );
        ...
        maxWallet = totalTokenSupply / 25; // 4%
    ...
    // Enabled or disable blacklisting
    function setBlacklisting(bool _blacklist) external onlyOwner {
        blacklisting = _blacklist;
    }
...
contract RWRD_BDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        RWRD_BContract = RWRD_B(mainContract);
    ...
    function _transfer(
        ...
        require(false, "RWRD_B_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "RWRD_B_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RWRD_B contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(RWRD_BContract);

            uint256 prevBalance = RWRD_BContract.balanceOf(address(this));
            ...
            {
                uint256 received = RWRD_BContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RWRD_BContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
